...
START_TEST(int_to_dec_22) {
  int check = s21_from_int_to_decimal(5, NULL);
...
START_TEST(dec_to_float_37) {
  ...
  int result = 0;
  result = s21_from_decimal_to_float(number, NULL);
...
START_TEST(s21_from_float_to_decimal_3) {
  s21_decimal val;
  s21_from_float_to_decimal(1.02E+08F, &val);
  ck_assert_int_eq(val.bits[0], 102000000);
  ck_assert_int_eq(val.bits[1], 0);
  ck_assert_int_eq(val.bits[2], 0);
...
void srunner_add_another_tests(SRunner *sr) {
  Suite *s1 = suite_create("Another");
  TCase *tc1_1 = tcase_create("Another");
  suite_add_tcase(s1, tc1_1);
...
void srunner_add_cast_tests(SRunner *sr) {
  ...
  tcase_add_test(tc1_1, dec_to_float_37);
